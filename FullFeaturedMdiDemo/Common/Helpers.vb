...
Namespace Common
    Public NotInheritable Class Helpers
        ...
        Public Shared Sub SerializeOptions(ByVal path As String, ByVal dbView As DatabaseSchemaView, ByVal childForm As ChildForm)
            Using fileStream As FileStream = New FileStream(path, FileMode.Create, FileAccess.ReadWrite)
                Using xmlBuilder As XmlDescriptionBuilder = New XmlDescriptionBuilder(fileStream)
                    Dim service As OptionsSerializationService = New OptionsSerializationService(xmlBuilder) With {.SerializeDefaultValues = True}
                    Dim metadataService As MetadataSerializationService = New MetadataSerializationService(xmlBuilder)
                    XmlSerializerExtensions.Builder = xmlBuilder
                    Dim root As Object = xmlBuilder.BeginObject("Options")
                    If True Then
                        ' Behavior options
                        Dim behaviorHandle As Object = xmlBuilder.BeginObjectProperty(root, Constants.BehaviorOptionsTag)
                        If True Then
                        ...
                        Dim dbViewOptionsHandle As Object = xmlBuilder.BeginObjectProperty(root, Constants.DatabaseSchemaViewOptionsTag)
                        If True Then
                            service.EncodeObject(dbViewOptionsHandle, dbView.Options)
                        End If
                        xmlBuilder.EndObjectProperty(dbViewOptionsHandle)
                        ' DesignPaneOptions
                        Dim designPaneOptionsHandle As Object = xmlBuilder.BeginObjectProperty(root, Constants.DesignPaneOptionsTag)
                        If True Then
                        ...
                        ' VisualOptions
                        Dim visualOptionsHandle As Object = xmlBuilder.BeginObjectProperty(root, Constants.VisualOptionsTag)
                        If True Then
                        ...
                        Dim addObjectDialogHandle As Object = xmlBuilder.BeginObjectProperty(root, Constants.AddObjectDialogOptionsTag)
                        If True Then
                        ...
                        ' DataSourceOptions
                        Dim dataSourceOptionsHandle As Object = xmlBuilder.BeginObjectProperty(root, "DataSourceOptions")
                        If True Then
                        ...
                        Dim metadataLoadingOptionsHandle As Object = xmlBuilder.BeginObjectProperty(root, "MetadataLoadingOptions")
                        If True Then
                        ...
                        Dim metadataStructureOptionsHandle As Object = xmlBuilder.BeginObjectProperty(root, "MetadataStructureOptions")
                        If True Then
                        ...
                        Dim queryColumnListOptionsHandle As Object = xmlBuilder.BeginObjectProperty(root, "QueryColumnListOptions")
                        If True Then
                        ...
                        ' QueryNavBarOptions
                        Dim queryNavBarOptionsHandle As Object = xmlBuilder.BeginObjectProperty(root, "QueryNavBarOptions")
                        If True Then
                        ...
                        Dim userInterfaceOptionsHandle As Object = xmlBuilder.BeginObjectProperty(root, "UserInterfaceOptions")
                        If True Then
                        ...
                        Dim sqlFormattingOptionsHandle As Object = xmlBuilder.BeginObjectProperty(root, "SqlFormattingOptions")
                        If True Then
        ...
        Public Shared Sub DeserializeOptions(ByVal xml As String, ByVal dbView As DatabaseSchemaView, ByVal childForm As ChildForm)
            ...
            Using memoryStream As MemoryStream = New MemoryStream(buffer)
                ...
                Dim metadataService As MetadataDeserializationService = New MetadataDeserializationService(adapter)

                adapter.Reader.ReadToFollowing(Constants.BehaviorOptionsTag)
                Dim behaviorOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
                behaviorOptionsTree.Read()
                service.DecodeObject(behaviorOptionsTree, childForm.BehaviorOptions)
                behaviorOptionsTree.Close()
                adapter.Reader.Read()

                Dim dbViewOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
                dbViewOptionsTree.Read()
                service.DecodeObject(dbViewOptionsTree, dbView.Options)
                dbViewOptionsTree.Close()
                adapter.Reader.Read()

                Dim designPaneOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
                ...
                Dim visualOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
                visualOptionsTree.Read()
                service.DecodeObject(visualOptionsTree, childForm.VisualOptions)
                visualOptionsTree.Close()
                adapter.Reader.Read()

                Dim addObjectDialogOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
                ...
                Dim dataSourceOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
                ...
                Dim metadataLoadingOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
                ...
                Dim metadataStructureOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
                ...
                Dim queryColumnListTree As XmlReader = adapter.Reader.ReadSubtree()
                ...
                Dim queryNavBarTree As XmlReader = adapter.Reader.ReadSubtree()
                queryNavBarTree.Read()
                service.DecodeObject(queryNavBarTree, childForm.QueryNavBarOptions)
                queryNavBarTree.Close()
                adapter.Reader.Read()

                Dim userInterfaceOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
                ...
                Dim sqlFormattingOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
                ...
                Dim sqlGenerationOptionsTree As XmlReader = adapter.Reader.ReadSubtree()
